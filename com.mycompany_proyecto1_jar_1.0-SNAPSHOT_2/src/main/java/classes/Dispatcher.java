...
public class Dispatcher {
    ...
    public Dispatcher(List readyList, List blockedList, List exitList, List suspendedReadyList, List suspendedBlockedList, List allProcess, W1 window) {
        ...
        // FIXED: Calculate initial available memory correctly
    ...
    public ProcessImage getProcess(){
        ...
        if(!this.readyList.isEmpty()){
            ...
            // Check suspension/resumption BEFORE getting a process
    ...
    private void sortReadyQueue(int schedulingAlgorithm) {
        switch (schedulingAlgorithm) {
            ...
            case 4:
                ...
                break;
            case 5:
                // FIXED: For Feedback, use priority (lower waiting time = higher priority)
    ...
    // FIXED: Priority should favor processes with HIGHER waiting time (they've waited longer)
    private List sortByPriority(List list) {
        return bubbleSort(list, (p1, p2) -> Integer.compare(
                ((ProcessImage) p2).getWaitingTime(),  // Higher waiting time first
    ...
    // FIXED: More aggressive suspension logic with better logging
    private void checkMemoryAndSuspend() {
        // Trigger when memory is low (less than 300 MB available)
        if (availableMemory < 300) {
            System.out.println("\n=== MEMORY LOW: " + availableMemory + " MB available. Starting suspension... ===");
            
            // Suspend ready processes first (prioritize largest processes)
            ...
            // Suspend blocked processes if still needed
    ...
    // FIXED: Resume suspended processes when memory becomes available
    private void checkMemoryAndResume() {
        // Resume suspended-ready processes first
        ...
        // Resume suspended-blocked processes
